...
interface FolderTarget {
  /**
   * Set when the target folder does not exist yet. `relativePath` is created
   * inside `parent` (or the drive root when `parent` is null).
   */
...
function resolveFolderTarget(
  ...
  // Creating a new folder. The AI names the existing folder to nest inside via
  // parentFolderId; a full path that starts with a known folder's path is also
  // resolved beneath that folder so subfolders land where the user's folders
  // live instead of at the drive root.
  ...
  const target = parentFolder
    ? resolveFolderPathTarget({
        folderPath: joinFolderPath(
          parentFolder.path || parentFolder.name,
          requestedPath,
        ),
        folders: folders.filter(
          (f) => f.driveConnectionId === parentFolder.driveConnectionId,
        ),
      })
    : resolveFolderPathTarget({ folderPath: requestedPath, folders });
  ...
  const resolvedParent =
    target.kind === "create" ? (target.parent ?? parentFolder ?? null) : null;
